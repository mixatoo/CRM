...
import { CLIENTS_TABLE_COLUMN_OPTIONS } from '@/features/clients/components/list/clients-table-columns'
...
type ClientsTableColumnPickerProps = {
  layout: ClientsTableLayout
  className?: string
}

export function ClientsTableColumnPicker({ layout, className }: ClientsTableColumnPickerProps) {
  const { toggleableColumns, visibleToggleableCount, reorderableVisibleKeys, isVisible, toggle, moveColumn, reset, isDefaultLayout } =
    layout
  ...
  const columnLabels = useMemo(
    () => new Map(CLIENTS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
