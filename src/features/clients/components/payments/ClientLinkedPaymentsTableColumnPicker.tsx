...
import { CLIENT_LINKED_PAYMENTS_TABLE_COLUMN_OPTIONS } from '@/features/clients/components/payments/client-linked-payments-table-columns'
...
type ClientLinkedPaymentsTableColumnPickerProps = {
  layout: ClientLinkedPaymentsTableLayout
  className?: string
}

export function ClientLinkedPaymentsTableColumnPicker({
  layout,
  className,
}: ClientLinkedPaymentsTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(CLIENT_LINKED_PAYMENTS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
