...
import { CLIENT_LINKED_SERVICES_TABLE_COLUMN_OPTIONS } from '@/features/clients/components/services/client-linked-services-table-columns'
...
type ClientLinkedServicesTableColumnPickerProps = {
  layout: ClientLinkedServicesTableLayout
  className?: string
}

export function ClientLinkedServicesTableColumnPicker({
  layout,
  className,
}: ClientLinkedServicesTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(CLIENT_LINKED_SERVICES_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
