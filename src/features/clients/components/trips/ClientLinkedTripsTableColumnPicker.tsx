...
import { CLIENT_LINKED_TRIPS_TABLE_COLUMN_OPTIONS } from '@/features/clients/components/trips/client-linked-trips-table-columns'
...
type ClientLinkedTripsTableColumnPickerProps = {
  layout: ClientLinkedTripsTableLayout
  className?: string
}

export function ClientLinkedTripsTableColumnPicker({
  layout,
  className,
}: ClientLinkedTripsTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(CLIENT_LINKED_TRIPS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
