...
import { SUPPLIER_STATUSES, SUPPLIER_STATUS_LABELS, type SupplierStatus } from '@/domain/entities/supplier'
...
interface SupplierStatusFilterProps {
  ...
  onChange: (value: SupplierStatus | 'all') => void
}

export function SupplierStatusFilter({ value, onChange }: SupplierStatusFilterProps) {
  const activeLabel = value === 'all' ? 'All statuses' : SUPPLIER_STATUS_LABELS[value]
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        ...
        >
          ...
          <span className="hidden max-w-[7rem] truncate text-[11px] md:inline">
            {value === 'all' ? 'All' : SUPPLIER_STATUS_LABELS[value]}
          </span>
      ...
      <DropdownMenu.Portal>
        ...
        >
          ...
          <div className="max-h-[min(18rem,50vh)] overflow-y-auto p-1" role="radiogroup" aria-label="Supplier status">
            <button
              type="button"
              role="radio"
