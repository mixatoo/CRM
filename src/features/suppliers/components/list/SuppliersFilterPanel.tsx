...
import { Check, ChevronRight, Globe2, MapPin, SlidersHorizontal, Tag, Tags } from 'lucide-react'
...
import {
  countActiveSupplierPanelFilters,
  useSupplierFilterOptions,
  type SupplierCategoryFilterOption,
  type SupplierFilterOptionItem,
} from '@/features/suppliers/hooks/use-supplier-filter-options'
...
import { EMPTY_SUPPLIER_PANEL_FILTERS, type SupplierPanelFilters } from '@/repositories/interfaces'
import { cn } from '@/shared/utils/cn'

type FilterCategory = 'category' | 'country' | 'city' | 'labels'
...
const CATEGORIES: Array<{
  ...
  { id: 'category', label: 'Category', icon: Tags, description: 'Hotel, airline, DMC…' },
  { id: 'country', label: 'Country', icon: Globe2, description: 'Supplier country' },
  { id: 'city', label: 'City', icon: MapPin, description: 'Supplier city' },
  { id: 'labels', label: 'Labels', icon: Tag, description: 'Assigned supplier labels' },
]
...
function filtersEqual(a: SupplierPanelFilters, b: SupplierPanelFilters) {
  return (
...
interface SuppliersFilterPanelProps {
  ...
  filters: SupplierPanelFilters
  onFiltersChange: (filters: SupplierPanelFilters) => void
  matchCount?: number
  status?: SupplierStatus | 'all'
}

export function SuppliersFilterPanel({
  open,
  onOpenChange,
  filters,
  onFiltersChange,
  ...
  const options = useSupplierFilterOptions()
  const { data: availableLabels = [] } = useLabelsForTarget('supplier')
  const labelById = useMemo(
    () => new Map(availableLabels.map((label) => [label.id, label])),
    [availableLabels],
  )
  const [category, setCategory] = useState<FilterCategory>('category')
  const [categoryQuery, setCategoryQuery] = useState('')
  const [countryQuery, setCountryQuery] = useState('')
  const [cityQuery, setCityQuery] = useState('')
  const [draftFilters, setDraftFilters] = useState<SupplierPanelFilters>(filters)

  const hasDraftChanges = !filtersEqual(draftFilters, filters)
  const activeCount = countActiveSupplierPanelFilters(draftFilters)
  const appliedCount = countActiveSupplierPanelFilters(filters)
  const totalSuppliers = options.totalSuppliers
  ...
  }, [options.categories, categoryQuery])
  ...
  const activeChips = useMemo(() => {
    ...
    if (draftFilters.category !== 'all') {
      chips.push({
        key: 'category',
        label: SUPPLIER_CATEGORY_LABELS[draftFilters.category],
        onRemove: () => setDraftFilters((current) => ({ ...current, category: 'all' })),
      })
    }
  ...
  const update = (patch: Partial<SupplierPanelFilters>) => {
                ...
                <p className="mt-1 text-xs font-normal text-[var(--color-muted)]">
                  {hasDraftChanges ? (
                    ...
                    <>
                      ...
                      {' of '}
                      <span className="tabular-nums">{totalSuppliers}</span> suppliers
                      {status !== 'all' && (
                        <>
                          {' · '}
                    ...
                    'Refine by category, country, city, or labels'
                  )}
                </p>
              </div>
              ...
              <div className="flex min-h-0 flex-1 flex-col px-4 py-3">
                {category === 'category' && (
                  <CategoryPicker
                    ...
                    onChange={(nextCategory) => update({ category: nextCategory })}
                ...
                {category === 'labels' && (
                  <LabelFilterPanelSection
                    ...
                    targetType="supplier"
                    searchAriaLabel="Filter supplier labels by name"
                  />
                )}
              </div>
            </div>
          ...
          <footer className={cn('shrink-0 border-t border-[var(--color-border)] px-4 py-3', STICKY_BAR_CLASS)}>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => setDraftFilters(EMPTY_SUPPLIER_PANEL_FILTERS)}
              >
                Reset
              </Button>
              <Button
...
function CategoryPicker({
  ...
  value: SupplierPanelFilters['category']
  options: SupplierCategoryFilterOption[]
  totalSuppliers: number
  onChange: (value: SupplierPanelFilters['category']) => void
  emptyMessage: string
}) {
  return (
...
function FacetPicker({
  ...
  onQueryChange: (value: string) => void
  value: string
  options: SupplierFilterOptionItem[]
  totalSuppliers: number
