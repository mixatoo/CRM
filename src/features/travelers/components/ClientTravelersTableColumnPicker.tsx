...
import { CLIENT_TRAVELERS_TABLE_COLUMN_OPTIONS } from '@/features/travelers/components/client-travelers-table-columns'
...
type ClientTravelersTableColumnPickerProps = {
  layout: ClientTravelersTableLayout
  className?: string
}

export function ClientTravelersTableColumnPicker({
  layout,
  className,
}: ClientTravelersTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(CLIENT_TRAVELERS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
