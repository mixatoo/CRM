...
import { TRAVELERS_TABLE_COLUMN_OPTIONS } from '@/features/travelers/components/list/travelers-table-columns'
...
type TravelersTableColumnPickerProps = {
  layout: TravelersTableLayout
  className?: string
}

export function TravelersTableColumnPicker({ layout, className }: TravelersTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(TRAVELERS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
