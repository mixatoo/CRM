...
const SORT_OPTIONS: Array<{ field: TripSortField; label: string }> = [
...
function getSortDirLabels(field: TripSortField): { asc: string; desc: string } {
  switch (field) {
    case 'date':
...
function getActiveSortLabel(field: TripSortField, dir: TripSortDir): string {
...
export function TripsSortMenu({ sortBy, sortDir, onSort, onSortDirChange }: TripsSortMenuProps) {
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <Button
          ...
          aria-label={`Sort trips. ${activeLabel}`}
        >
      ...
      <DropdownMenu.Portal>
        ...
        >
          <div className="border-b border-[var(--color-border)] px-3 py-2.5">
            <p className="text-xs font-semibold text-[var(--color-foreground)]">Sort trips</p>
