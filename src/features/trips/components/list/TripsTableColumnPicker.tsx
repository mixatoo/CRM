...
import { TRIPS_TABLE_COLUMN_OPTIONS } from '@/features/trips/components/list/trips-table-columns'
...
type TripsTableColumnPickerProps = {
  layout: TripsTableLayout
  className?: string
}

export function TripsTableColumnPicker({ layout, className }: TripsTableColumnPickerProps) {
  const {
    toggleableColumns,
  ...
  const columnLabels = useMemo(
    () => new Map(TRIPS_TABLE_COLUMN_OPTIONS.map((column) => [column.key, column.label])),
    [],
  )
