...
import { formatPersonCount, formatTripDate, tripClientName } from '@/features/trips/utils/format'
...
function TripClientCell({ trip }: { trip: Trip }) {
  const name = tripClientName(trip)
  const paletteKey = trip.clientId || trip.id
  const initials = accountNameInitials(name === '—' ? 'Client' : name)
  const palette = ACCOUNT_NAME_AVATAR_PALETTE[accountNameAvatarPaletteIndex(paletteKey)]
...
function tripsHeadClass(extra?: string) {
...
function SortableHeader({
  ...
  return (
    <DataTableColumnHeader
      label={label}
      align={align}
      className={tripsHeadClass(className)}
      active={active}
      sortDir={sortDir}
      onSort={() => onSort(field)}
...
export function renderTripsTableHeader(key: TripsTableColumnKey, ctx: HeaderContext): ReactNode {
  switch (key) {
    case 'selection':
      return (
        <th
    ...
    case 'labels':
      return (
        <DataTableColumnHeader
          ...
          align="left"
          sortable={false}
          className={tripsHeadClass(COMPACT_COLUMN_X)}
        />
      )
    case 'name':
      return (
...
export type TripsTableCellContext = {
  trip: Trip
  labels: Label[]
  selected: boolean
  isDeleting: boolean
...
export function renderTripsTableCell(key: TripsTableColumnKey, ctx: TripsTableCellContext): ReactNode {
  ...
  switch (key) {
    ...
    case 'client':
      return (
        <td key={key} className={tableCellClass('left')} title={tripClientName(ctx.trip)}>
          <TripClientCell trip={ctx.trip} />
        </td>
      )
    case 'persons':
      return (
