...
const SORT_OPTIONS: Array<{ field: TripServiceSortField; label: string }> = [
  { field: 'lineNumber', label: '#' },
  { field: 'startDate', label: 'Date' },
  { field: 'category', label: 'Service' },
  { field: 'supplierName', label: 'Supplier' },
  { field: 'cost', label: 'Cost' },
  { field: 'selling', label: 'Selling' },
  { field: 'margin', label: 'Margin' },
  { field: 'status', label: 'Status' },
]

function getSortDirLabels(field: TripServiceSortField): { asc: string; desc: string } {
  switch (field) {
    case 'lineNumber':
...
function getActiveSortLabel(field: TripServiceSortField, dir: TripServiceSortDir): string {
...
interface TripServicesSortMenuProps {
...
export function TripServicesSortMenu({
  sortBy,
  sortDir,
  onSort,
  onSortDirChange,
}: TripServicesSortMenuProps) {
  const dirLabels = getSortDirLabels(sortBy)
  const activeLabel = getActiveSortLabel(sortBy, sortDir)
  const isDefaultSort = sortBy === 'lineNumber' && sortDir === 'asc'
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <Button
          ...
          aria-label={`Sort services. ${activeLabel}`}
        >
      ...
      <DropdownMenu.Portal>
        ...
        >
          <div className="border-b border-[var(--color-border)] px-3 py-2.5">
            <p className="text-xs font-semibold text-[var(--color-foreground)]">Sort services</p>
