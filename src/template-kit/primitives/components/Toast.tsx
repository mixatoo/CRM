      ...
      <div className="flex gap-3 px-3.5 py-3.5 pr-3">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          {item.cta ? (
            <button
              type="button"
              className="mt-1.5 inline-flex items-center gap-1 text-xs font-semibold text-white/90 transition-colors hover:text-white"
              onClick={() => {
                item.cta?.onClick()
                onDismiss(item.id)
              }}
        ...
        <ToastPrimitive.Close asChild>
          <button
            type="button"
            aria-label="Dismiss"
            className="relative flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full text-[var(--color-sidebar-text)] transition-colors hover:text-white"
          >
